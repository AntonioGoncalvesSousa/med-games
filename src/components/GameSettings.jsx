...
export default function GameSettings({ game, onStart, onBack }) {
  ...
  const submit = () => {
    ...
    onStart({ mode, allowSkip, randomOrder, selectedRegions });
  };

  return (
    <main className="settings-page page-enter">
      <button className="back-link" onClick={onBack}>← Voltar ao início</button>
      <div className="settings-heading">
        <p className="eyebrow">Preparação da sessão</p>
        <h1>Como você quer <em>estudar?</em></h1>
        <p>Escolha o formato e o recorte anatômico para montar sua rodada.</p>
      </div>
      ...
      <button className="primary-button start-button" onClick={submit}>Começar rodada <span>→</span></button>
    </main>
  );
}
